...
| [`cg-auto-run`](../skills/cg-auto-run/SKILL.md) | **Opt-in.** Chain lifecycle stages without returning each hop to the user: dispatch one stage, follow its `Next action` route only while the status advances and the granted authority allows it, and stop on any blocker, `cg-unblock` route, or exhausted stage budget. Does no lifecycle work itself. |
