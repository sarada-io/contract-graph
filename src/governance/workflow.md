...
6. **Decide, don't ask live.** Unspecified detail is resolved from the principles, the existing
   ...
   `.agents/skills/cg-decide/SKILL.md`: it is binding for every non-trivial task. Its Rule D-4
...
7. Use `cg-execute` to run the earliest `Ready` Step, one at a time. After each verified handoff,
...
10. Use `cg-complete` to verify the accumulated Step sequence, drive defects through corrective
...
change to the Step that changes the behavior; `cg-execute` changes the contract and detector with
the implementation; `cg-complete` verifies the accumulated result. When completion finds a
current-phase defect, it owns the repair loop: completion-only composition work is fixed directly,
while behavior- or contract-affecting work is run through a corrective `cg-execute` Step.
`cg-prepare` is used first only when the remaining order or editable paths must change.
`cg-document` never repairs contract truth, and `cg-complete` never edits a contract outside an
execution Step.

`cg-complete` may create a successor handover for `cg-plan` or `cg-prepare` only when the finding
...
deleted), follow `.agents/skills/cg-complete/SKILL.md`. In short:
