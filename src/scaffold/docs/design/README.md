...
distinction between the two: a plan records what you are about to do, a design record states what
...
record under `docs/plans/`. Nor binding rules: a rule belongs in `.agents/cg/principles/` with a
detector, and a design record that quietly becomes a rule is a rule nothing enforces.
