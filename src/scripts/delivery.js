...
import { encodeDelivery, decodeDelivery } from "./delivery-storage.js";

export const DELIVERY_ROOT = ".agents/cg/deliveries";
const RECORD_ROOTS = [DELIVERY_ROOT, ".agents/cg/prototypes"];
export const DELIVERY_STATES = ["Iterating", "Awaiting review", "Approved", "Handed off", "Suspended", "Abandoned", "Closed"];
...
export function deliveryDocs(root) {
...
function ownedPath(root, relative) {
  ...
  for (const part of parts) {
    ...
    try {
      if (fs.lstatSync(current).isSymbolicLink()) throw new Error(`delivery metadata cannot follow a symlink: ${relative}`);
 ...
 */
export function deliverySnapshot(root, writes) {
...
// remove delivery code from review. Parent directories subsume their explicitly listed files.
 ...
 */
export function deliveryReviewSnapshot(root, record) {
  return deliverySnapshot(root, record.reviewScope ? reviewWrites(record) : undefined);
}

export function validateDelivery(record, file) {
  try { record = decodeDelivery(record); } catch (error) { throw new Error(`${file}: ${error.message}`); }
  if (!record || record.version !== 1 || typeof record.programme !== "string" || !slugPattern.test(record.programme) ||
      !DELIVERY_STATES.includes(record.status) || !digestPattern.test(record.baseline ?? "") ||
      !Array.isArray(record.history) || !record.history.length ||
      record.history.some((event) => !DELIVERY_STATES.includes(event.status) || !nonempty(event.at)) ||
      record.history.at(-1).status !== record.status) throw new Error(`${file}: malformed delivery record`);
  if (["Approved", "Handed off", "Closed"].includes(record.status)) {
    ...
    if (!approval || ![approval.by, approval.response, approval.scope, approval.at].every(nonempty) ||
        !digestPattern.test(approval.snapshot ?? "")) throw new Error(`${file}: missing attributed delivery approval`);
  }
  ...
  if (record.sessions !== undefined && (!Array.isArray(record.sessions) || record.sessions.some(s =>
    ...
    throw new Error(`${file}: malformed delivery session checkpoint`);
  }
  if ((record.reviewScope !== undefined && (!Array.isArray(record.reviewScope) || !record.reviewScope.length || record.reviewScope.some(p => !writePath(p)))) ||
    ...
    throw new Error(`${file}: malformed delivery review scope`);
  }
  ...
  if (record.handoff && (!nonempty(record.handoff.roadmap) || !nonempty(record.handoff.scope) ||
      !digestPattern.test(record.handoff.sourceSnapshot ?? "") ||
      record.handoff.reviewSnapshot !== record.approval?.snapshot)) {
    throw new Error(`${file}: malformed accepted delivery handoff`);
  }
  if (record.completionRequest !== undefined) {
    ...
    if (!request || !["Active", "Completed"].includes(request.state) ||
      ...
      throw new Error(`${file}: malformed delivery completion request`);
    }
  }
  return record;
}

export function readDeliveries(root) {
  const records = RECORD_ROOTS.flatMap(folder => {
    const directory = ownedPath(root, folder);
    ...
    return fs.readdirSync(directory).sort().map((name) => {
      ...
      if (!/^[a-z0-9]+(?:-[a-z0-9]+)*\.json$/.test(name)) throw new Error(`${folder}/${name}: unexpected delivery record`);
      const file = `${folder}/${name}`;
      const record = validateDelivery(JSON.parse(fs.readFileSync(ownedPath(root, file), "utf8")), file);
  ...
  for (const record of records) {
    if (names.has(record.programme)) throw new Error(`duplicate delivery record for ${record.programme}; reconcile the records without discarding evidence`);
    names.add(record.programme);
  }
  return records;
}

function recordPath(root, name) {
  return RECORD_ROOTS.map(folder => `${folder}/${name}.json`).find(file => fs.existsSync(ownedPath(root, file))) ?? `${DELIVERY_ROOT}/${name}.json`;
}
...
export function signOffRecovery(root) {
  const candidates = readDeliveries(root).filter(record => record.completionRequest?.state === "Active")
    .map(record => ({ programme: record.programme, mode: "delivery-completion", record: record.file,
...
function save(root, record, status, event = {}) {
  ...
  const file = ownedPath(root, recordPath(root, record.programme));
  ...
  try {
    ...
    const stored = legacy ? record : encodeDelivery(record);
    fs.writeFileSync(temp, `${JSON.stringify(stored)}\n`, { flag: "wx" });
    fs.renameSync(temp, file);
  } finally { fs.rmSync(temp, { force: true }); }
  return { ...record, file: recordPath(root, record.programme) };
}
...
function checkpoint(root, record, session, evidence) {
  ...
  const records = readDeliveries(root);
  ...
  const entry = { session, state: input.state, writes, resources, note: input.note, context: context(root),
    at: new Date().toISOString(), snapshot: deliverySnapshot(root), files, observedChanges, dirty, peers,
...
const STARTER_PHASES = "Finalise delivery phases after delivery acceptance.";
 ...
 */
export function deliveryAction(root, action, options = {}) {
  if (["start", "resume", "handoff", "close"].includes(action)) requireIntent(root);
  const { programme, session } = options;
  if (programme !== undefined) programmeName(programme);
  if (action === "status") return readDeliveries(root).filter(r => !programme || r.programme === programme);
  ...
  const lock = path.join(git(root, ["rev-parse", "--absolute-git-dir"]).trim(), `cg-delivery-${name}.lock`);
  try { fs.mkdirSync(lock); } catch (error) {
    if (error.code === "EEXIST") throw new Error(`delivery ${name} is being updated; inspect ${lock} and retry. Remove an interrupted lock only after confirming its owner has stopped`);
    throw error;
  }
  try {
...
function performAction(root, action, { programme: name, evidence, gate, session } = {}) {
  let record = readDeliveries(root).find(r => r.programme === name);
  ...
  if (evidence) {
    ...
    if (relative.startsWith(`${deliveryDocs(root)}/plans/${name}/`)) {
      const file = ownedPath(root, relative);
      if (!fs.statSync(file).isFile()) throw new Error("delivery evidence must name a file");
      event.evidence = relative;
    }
  }
  const persist = (status, extra = {}) => save(root, record, status, { ...event, ...extra });
  const roadmap = `${deliveryDocs(root)}/plans/${name}/roadmap.md`;
  if (action === "start") {
    if (record) throw new Error("delivery already exists; resume it instead of overwriting its history");
    const snapshot = deliverySnapshot(root);
  ...
  if (action === "checkpoint") {
    if (record.status === "Closed") throw new Error("resume a closed delivery before recording new session work");
  ...
  if (!allowed[action]?.includes(record.status)) throw new Error(`cannot ${action} delivery in ${record.status}`);
  if (action === "resume") {
    const previousEvidence = { approval: record.approval ?? null, closure: record.closure ?? null, completionRequest: record.completionRequest ?? null, handoff: record.handoff ?? null };
  ...
  if (action === "review") {
    ...
    record.review = deliveryReviewSnapshot(root, record);
    const docs = deliveryDocs(root);
  ...
  if (action === "approve") {
    ...
    const snapshot = deliveryReviewSnapshot(root, record);
    if (record.review !== snapshot) throw new Error("delivery changed after review; resume and present the current result");
  ...
  if (action === "handoff") {
    if (deliveryReviewSnapshot(root, record) !== record.approval.snapshot) throw new Error("approved delivery changed; resume for affected human review");
    const plan = fs.readFileSync(ownedPath(root, roadmap), "utf8");
    validateRoadmap(plan);
    record.handoff = { roadmap, scope: record.approval.scope, reviewSnapshot: record.approval.snapshot,
      sourceSnapshot: deliverySnapshot(root), obligations: `${roadmap}#deferred-tests-and-known-gaps`,
      completionGate: `${roadmap}#programme-completion-gate` };
    return persist("Handed off", { handoff: record.handoff });
  }
  if (action === "request-sign-off") {
    ...
    if (record.completionRequest?.state === "Active") throw new Error("delivery completion is already requested; resume the recorded work without replacing its authority");
    record.completionRequest = { by: request.by, response: request.response, scope: request.scope,
      session, state: "Active", at: new Date().toISOString(), snapshot: deliverySnapshot(root) };
  ...
  if (action === "close") {
    ...
    const before = deliverySnapshot(root);
    const attempt = { session: session ?? null, context: context(root), startedAt: new Date().toISOString(),
      ...
      otherProgrammes: readDeliveries(root).filter(r => r.programme !== name).map(r => ({ programme: r.programme, status: r.status })) };
    ...
    try { attempt.finalSnapshot = deliverySnapshot(root); } catch (error) {
...
export function deliveryReadiness(root, { base, gate } = {}) {
  const failures = [];
  // Installed repositories require adoption independently of delivery receipt presence.
  try { requireIntent(root); } catch (error) { failures.push(error.message); }
  if (!nonempty(gate)) return { failures: ["a trusted --gate command is required to validate delivery receipts"] };
  let records;
  try { records = readDeliveries(root); } catch (error) { return { failures: [error.message] }; }
  let affected = records;
  if (base) {
    if (git(root, ["rev-parse", "--is-shallow-repository"]).trim() === "true") throw new Error("delivery verification requires fetched history; shallow clones cannot detect removed delivery records");
    const commit = git(root, ["rev-parse", "--verify", `${base}^{commit}`]).trim();
    const history = git(root, ["log", "--format=", "--name-only", `${commit}..HEAD`, "--", ...RECORD_ROOTS]);
    const changed = git(root, ["diff", "--name-only", commit, "--", ...RECORD_ROOTS]);
    const names = new Set(`${history}\n${changed}`.split("\n").filter(name => RECORD_ROOTS.some(folder => name.startsWith(`${folder}/`))));
    ...
    for (const file of names) if (!records.some(r => r.file === file)) failures.push(`${file}: delivery record was removed; retain its delivery obligation`);
    affected = records.filter(r => names.has(r.file) || r.status !== "Closed");
  }
  const snapshot = affected.length ? deliverySnapshot(root) : null;
  for (const record of affected) {
    if (record.status !== "Closed") failures.push(`${record.programme}: Delivery — ${record.status}; final sign-off is required before merge`);
