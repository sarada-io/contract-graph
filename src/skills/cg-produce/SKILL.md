...
Before starting a run, ask: **“Implement all remaining items and review them together, or review after each developed item?”** All remaining means this authorised sprint/run scope. An explicit answer already supplied for this run satisfies the question. Save the actual choice, scope and response under `## Production run` in the same master roadmap; recover it after interruption rather than ask again. No answer means the choice is pending, not permission to pick a mode. Independent inspection may continue while waiting.
