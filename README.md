...
```bash
npx contract-graph init . --design saas,ops --profile all
cg sync
cg verify
```
...
.agents/
  cg/
    ...
    map/
      ...
      profile.json        selected editor profiles and design packs (generated by init)
...
| Command | Does |
|---|---|
| `cg init [dir] --design a,b --profile x,y` | scaffold governance and selected editor discovery; defaults to `all` |
...
The governance under `.agents/` is identical for every profile. A profile changes only the files
an editor uses to discover that governance. The selection is recorded in
