...
Every command below was run for 0.2.0. The checks exist because each one caught something real
...
`files` in `package.json` is an allowlist plus exclusions for the development CLI and this
maintainer runbook. Confirm nothing private leaked and that the exclusions held:
...
const bad=j.files.filter(f=>/^(plan|tmp|build)\//.test(f.path)||f.path.includes('dev.js')||f.path==='docs/RELEASING.md');
console.log('leaked:',bad.length?bad.map(f=>f.path).join(', '):'none');})"
```

`plan/`, `tmp/`, and `build/` are gitignored and must never appear. 0.2.0 ships **66 files,
466 KB**; a sudden jump means something was added to `files` or a directory grew.
...
Both shapes must reach `cg verify: OK`. Greenfield keeps the starter `src` module (42 files
written); brownfield skips it and reports the ungoverned module roots instead (39 files). Those
...
`fileCount` and `unpackedSize` must match what step 2 reported. 0.2.0: 66 files, 477147 bytes.
